...
constexpr ec_pdo_entry_info_t MDXServoEC::kPdoEntries_[];
constexpr ec_pdo_info_t MDXServoEC::kPDOs_[];
...
std::string MDXServoEC::getDriveStateStr(const std::bitset<16>& status_word)
{
...
RetVal MDXServoEC::sdoRequests(ec_slave_config_t* config_ptr)
{
...
void MDXServoEC::readInputs()
{
  ...
  if (drive_state_ != GetCurrentState())
  {
    if (drive_state_ == ST_OPERATION_ENABLED)
    {
      // Get target default
      MDXServoECData* data =
        new MDXServoECData(input_pdos_.display_op_mode, input_pdos_);
...
bool MDXServoEC::isReadyToShutDown() const
{
  return prev_state_ <= ST_SWITCH_ON_DISABLED;
}

void MDXServoEC::ecPrintCb(const std::string& msg,
...
void MDXServoEC::enableOperation()
{
...
void MDXServoEC::disableOperation()
{
...
void MDXServoEC::disableVoltage()
{
...
void MDXServoEC::changeDeltaPosition(const int32_t delta_position)
{
...
void MDXServoEC::changeDeltaVelocity(const int32_t delta_velocity)
{
...
void MDXServoEC::changeDeltaTorque(const int16_t delta_torque)
{
  changeTorque(input_pdos_.torque_actual_value + delta_torque);
}

void MDXServoEC::changeDigitalOutputs(const uint32_t digital_outputs)
{
  MDXServoECData* data = new MDXServoECData(
...
void MDXServoEC::changeOpMode(const int8_t target_op_mode,
                                        const bool verbose /*=false*/)
{
  printCommand("ChangeOpMode");
  // Set target value to current one
  MDXServoECData* data =
    new MDXServoECData(target_op_mode, input_pdos_, true);
  if (verbose)
    printTarget(*data);
  setChange(data);
}

void MDXServoEC::setTargetDefaults(const bool verbose /*=false*/)
{
  printCommand("SetTargetDefaults");
  // Set target operational mode and value to current ones
  MDXServoECData* data =
    new MDXServoECData(input_pdos_.display_op_mode, input_pdos_, true);
  if (verbose)
    printTarget(*data);
  // Apply
  setChange(data);
...
void MDXServoEC::setChange(const MDXServoECData* data)
{
  // clang-format off
...
STATE_DEFINE(MDXServoEC, Start, NoEventData)
{
...
STATE_DEFINE(MDXServoEC, NotReadyToSwitchOn, NoEventData)
{
...
inline void MDXServoEC::printCommand(const char* cmd) const
{
  std::string msg;
#if USE_QT
...
inline void MDXServoEC::printTarget(const int target) const
{
  std::string msg;
#if USE_QT
...
inline void MDXServoEC::printTarget(const MDXServoECData& data) const
{
  std::string msg;
#if USE_QT
...
void MDXServoEC::printStateTransition(const States current_state,
