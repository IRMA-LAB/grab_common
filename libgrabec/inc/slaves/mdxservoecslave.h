/**
 * @file mdxservoecslave.h
 * @author Filippo Zoffoli
 * @date Jan 2022
 * @brief File containing _MDX+Servo EthercAT_ slave interface to be included in the
 * GRAB ethercat library.
 */

#ifndef GRABCOMMON_LIBGRABEC_MDXSERVOECSLAVE_H
#define GRABCOMMON_LIBGRABEC_MDXSERVOECSLAVE_H
...
namespace grabec {
/**
 * @brief MDX+ Servo Ethercat _operation modes_.
 *
 ...
 */
struct MDXServoECInPdos
{
  std::bitset<16> error_code; /**< error_code */
...
/**
 * @brief The MDXServoECData class.
 *
...
class MDXServoECData: public EventData
{
 public:
   ...
   * MDXServoECOperationModes for valid accounted entries.
   ...
   */
  MDXServoECData(const int8_t _op_mode, const int32_t _value = 0);
  /**
   ...
   */
  MDXServoECData(const int8_t _op_mode, const MDXServoECInPdos& input_pdos,
   ...
   * in range [–32,768, 32,767].
   */
   MDXServoECData(const int8_t _op_mode, const uint32_t _digital_outputs,
...
/**
 * @brief The MDX+ Servo EtherCAT class.
 *
  ...
  public StateMachine
{
 ...
 public:
  /**
   * @brief MDX+ Servo EtherCAT _states_ as in physical drive documentation.
   *
   */
  enum States : BYTE
  {
 ...
 public:
   ...
   */
  MDXServoECInPdos getDriveStatus() const { return input_pdos_; }
  ...
  /**
   * @defgroup ExternalEvents MDXServoEC External Events
   * The external events which user can call to trigger a state transition.
   * @{
   */
 ...
 private:
  ...
  static constexpr uint16_t kErrorCodeIdx           = 0x603F;
  static constexpr uint8_t kErrorCodeSubIdx         = 0x00;
  ...
  void setChange(const MDXServoECData *data);
  ...
  inline void printTarget(const MDXServoECData& data) const;
 ...
 private:
  ...
  STATE_DECLARE(MDXServoEC, Start, NoEventData)
  STATE_DECLARE(MDXServoEC, NotReadyToSwitchOn, NoEventData)
  STATE_DECLARE(MDXServoEC, SwitchOnDisabled, NoEventData)
  STATE_DECLARE(MDXServoEC, ReadyToSwitchOn, NoEventData)
  STATE_DECLARE(MDXServoEC, SwitchedOn, NoEventData)
  STATE_DECLARE(MDXServoEC, OperationEnabled, MDXServoECData)
  STATE_DECLARE(MDXServoEC, QuickStopActive, NoEventData)
  STATE_DECLARE(MDXServoEC, FaultReactionActive, NoEventData)
  STATE_DECLARE(MDXServoEC, Fault, NoEventData)
...
#endif // GRABCOMMON_LIBGRABEC_MDXSERVOECSLAVE_H
