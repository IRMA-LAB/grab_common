...
""")
    ...
    f.write("""
  // Ethercat utilities, describing index, subindex and bit length of each configured PDO entry.
...
""")
  ...
  // Ethercat utilities, describing memory position of input and output PDOs stack.
""")
    ...
    if outputs_present:
        ...
        f.write('    {%s, %d, const_cast<ec_pdo_entry_info_t*>(kPdoEntries_)}, /**< Output PDOs */\n'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Parse a .XML file produced by'\
            ' EasyCAT_Config_GUI and generate corresponding header and source file to include custom device in'\
            ' an EtherCAT network.')
    ...
    parser.add_argument('-o','--ofile', dest='ofile',
                        help='output class file name suffix (not path). If not given, input file name will be used')
    ...
    if parsed_args.ofile is None:
        ofilename = os.path.splitext(os.path.split(parsed_args.ifile)[1])[0] + '_slave'
    else:
        ofilename = parsed_args.ofile

    # Generate class files.
    print('Generating header file...')
    gen_header(os.path.join(os.path.dirname(__file__), '..\\inc\\slaves', ofilename + '.h'), config_params)
    print('Generating source file...')
    gen_source(os.path.join(os.path.dirname(__file__), '..\\src\\slaves', ofilename + '.cpp'), config_params)
    print('Generation complete.\nYou can find the new files in the "slaves" folders of your local "libgrabec" directory.')
    print("Don't forget to copy '%s.h' in '%s'!" % (os.path.splitext(parsed_args.ifile.split('\\')[-1])[0],
            os.path.abspath(os.path.join(os.path.dirname(__file__), '..\\inc\\slaves'))))
