                                         ...
                                         const grabnum::Vector3d& transl_vect)
{
  ...
  homg_mat.SetBlock<3, 3>(0, 0, rot_mat);
  homg_mat.SetBlock<3, 1>(0, 3, transl_vect);
  return homg_mat;
}
...
grabnum::Matrix4d InverseTransformation(const grabnum::Matrix4d& homg_transf)
{
  ...
  inv_trans.SetBlock<3, 3>(0, 0, rot_mat_inv);
  inv_trans.SetBlock<3, 1>(0, 3, -rot_mat_inv * GetHomgTransfTransl(homg_transf));
  return inv_trans;
}
