 ...
 */
 // to recover the cpp delete "/*COMMENT WHOLE" across all the file (find-replace)
/*COMMENT WHOLE
...
UnderActuatedPlatformVars::UnderActuatedPlatformVars(
  const arma::uvec6& _mask /*=arma::uvec6(arma::fill::ones)*//*COMMENT WHOLE)
{
  setMask(_mask);
}

UnderActuatedPlatformVars::UnderActuatedPlatformVars(
  const PlatformVars vars, const arma::uvec6& _mask /*=arma::uvec6(arma::fill::ones)*//*COMMENT WHOLE)
  : PlatformVars(vars)
{
  setMask(_mask);
}
...
UnderActuatedPlatformVars::UnderActuatedPlatformVars(
  const RotParametrization _angles_type /*=TILT_TORSION*//*COMMENT WHOLE,
  const arma::uvec6& _mask /*=arma::uvec6(arma::fill::ones)*//*COMMENT WHOLE)
...
UnderActuatedRobotVars::UnderActuatedRobotVars(
  const size_t num_cables, const arma::uvec6& _mask /*=arma::uvec6(arma::fill::ones)*//*COMMENT WHOLE)
...
bool solveDK0GS(const std::vector<double>& cables_length,
                ...
                const uint8_t nmax /*= 100*//*COMMENT WHOLE, uint8_t* iter_out /*= nullptr*//*COMMENT WHOLE)
{
...
arma::vec nonLinsolveJacGeomStatic(const grabnum::VectorXd<POSE_DIM>& init_guess,
                                   ...
                                   const uint8_t nmax /*= 100*//*COMMENT WHOLE,
                                   uint8_t* iter_out /*= nullptr*//*COMMENT WHOLE)
{
                                           ...
                                           uint8_t* iter_out /*= nullptr*//*COMMENT WHOLE)
{
  ...
  while (iter < nmax && arma::norm(func_val) > kFtol && err > cond)
  {
    iter++;
    //bool X=true;
    /*X =*/ /*COMMENT WHOLEarma::solve(s,func_jacob, func_val);
...
arma::mat toArmaMat_3x6(Matrix<double, 3, 6> mat, bool copy /*= true*//*COMMENT WHOLE)
{
